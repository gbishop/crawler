...
export default class World {
  ...
  separate(body1, body2, processCallback, callbackContext, overlapOnly) {
    ...
    if (
      processCallback &&
      processCallback.call(callbackContext, body1.sprite, body2.sprite) ===
        false
    ) {
      return false;
    }
    ...
    if (
      ...
      Math.abs(this.gravity.z + body1.gravity.z) <
        Math.abs(this.gravity.y + body1.gravity.y)
    ) {
      this._result =
  ...
  separateX(body1, body2, overlapOnly) {
    ...
    this._maxOverlap =
      body1.deltaAbsX() + body2.deltaAbsX() + this.OVERLAP_BIAS;
    ...
    if (this._overlap !== 0) {
      ...
      if (!body1.immovable && !body2.immovable) {
        ...
        this._newVelocity1 =
          Math.sqrt(
            (this._velocity2 * this._velocity2 * body2.mass) / body1.mass
          ) * (this._velocity2 > 0 ? 1 : -1);
        this._newVelocity2 =
          Math.sqrt(
            (this._velocity1 * this._velocity1 * body1.mass) / body2.mass
  ...
  separateY(body1, body2, overlapOnly) {
    ...
    this._maxOverlap =
      body1.deltaAbsY() + body2.deltaAbsY() + this.OVERLAP_BIAS;
  ...
  separateZ(body1, body2, overlapOnly) {
    ...
    this._maxOverlap =
      body1.deltaAbsZ() + body2.deltaAbsZ() + this.OVERLAP_BIAS;
    ...
    if (sprite.body.skipTree || this.skipTree) {
      ...
      for (i = 0, len = this._potentials.length; i < len; i++) {
        ...
        if (
          ...
          if (collideCallback) {
            collideCallback.call(
              callbackContext,
              sprite,
              this._potentials[i].sprite
            );
          }
